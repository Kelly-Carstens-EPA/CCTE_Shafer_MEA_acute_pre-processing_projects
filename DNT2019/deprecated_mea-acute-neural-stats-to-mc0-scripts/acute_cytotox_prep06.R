...
createCytoTable = function(sourcefile,firstround) {
  ...
  if (isempty(AB_data)) {
    ...
    createCytoData(AB_data, "AB", firstround, plate.id, srcname, culture_date)
  }
  if(isempty(LDH_data)) {
    print("LDH data not found")
  } else {
    createCytoData(LDH_data, "LDH", 0, plate.id, srcname, culture_date)
  }
...
createCytoTable2 = function(sourcefile) {
  ...
  plate1_dat <- read.xlsx(sourcefile, sheetName = "Plate 1", header = F, endRow = 5, stringsAsFactors = F)
  ...
  for (i in 1:length(AB_plate_indicies)) {
    ...
    if (isempty(AB_data)) {
      ...
      AB_longdat <- createCytoData(AB_data, "AB", ABplate.id, srcname, culture_date, experiment_date)
    }
    if(isempty(LDH_data)) {
      print("LDH data not found")
    } else {
      LDH_longdat <- createCytoData(LDH_data, "LDH", LDHplate.id, srcname, culture_date, experiment_date)
    }
...
createCytoData = function(sourcedata, cyto_type, plate.id = NULL, srcname = "", culture_date = "", experiment_date) {
  ...
  if (sheetdata == "three") {
    tagPhrases = c("Percent of Control")
    valueindex = NULL
    i = 1
  ...
  # if any percent of control values are negative, then we should set these to zero
  # if (any(valuemap < 0)) {
  ...
  sourcedata$culture_date = culture_date
  ...
  # assign wllt
  sourcedata$wllt = NA # this will look different in every data set - will do separately

  # some clean up
  # will do this for each data set individually
  # # replace DMSOa, DMSOb, etc. with just 'DMSO'
  # sourcedata[grepl("DMSO",sourcedata$treatment), "treatment"] <- "DMSO"
  # 
  # # replace conc of "Control" or 0 with 1e-6
  # sourcedata[grepl("[Cc]ontrol",sourcedata$conc),"wllt"] = "n"
  
  # reorder columns
  sourcedata = sourcedata[,c("treatment","plate.id","apid","rowi","coli","wllt","wllq","conc","rval","srcf","acsn", "culture_date", "experiment_date")]
  
  return(sourcedata)
  
}

saveCytoData <- function(calc_files, dataset_title = "", output.dir) {
  ...
  filename <- file.path(output.dir, paste0("cytodat_",dataset_title,"_",as.character.Date(Sys.Date()),".RData"))
