...
checkFileNames <- function(check.dir = "", dataset_title = "", files_log = "") {
...
checkAnalysisTiming <- function(filei, file_col1, file_col2, threshold = 30.0) {
  
  message1 <- c()
  message2 <- c()
  # just checking duration and analyis start for now - might check file offset and such later
  # threshold determines how many seconds above or below 40 minutes the file may be
  # or how long after zero the file can start
  ...
  if (abs(as.numeric(analysis.duration) - 2400) > threshold) {
    message2 <- paste0(" analysis duration is ",analysis.duration,"s")
  }
  
  if (length(c(message1, message2))>0) {
    message <- paste0(basename(filei), paste(message1, message2, sep = ", "))
    return(message)
  }
  else {
    return(TRUE)
...
runChecks <- function(files, check.settings = T, check.parameters = T, check.timing = T, threshold = 30.0) {
  ...
  for (filei in files) {
    ...
    if (check.timing) {
      filei.timing <- checkAnalysisTiming(filei, file_col1, file_col2, threshold)
...
writeCheckSummary <- function(output.dir, dataset_title, check.settings = T, check.parameters = T, check.timing = T, threshold = 30.0) {
  ...
  # do the checks
  check.results <- runChecks(files, check.settings, check.parameters, check.timing, threshold)
  ...
  if (check.timing){
    ...
    cat("\nThere are",length(which(timing_summary != TRUE)),"files with analysis start after",threshold,
        "seconds or analysis duration at least",threshold,"seconds off of 40 minutes (2400s):\n")
