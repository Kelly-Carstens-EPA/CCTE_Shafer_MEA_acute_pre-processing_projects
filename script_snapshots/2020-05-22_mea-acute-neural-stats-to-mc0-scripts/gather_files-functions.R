...
read_files <- function(check.dir = "", log_file = "", files_type = "neural_stats") {
  
  if (log_file == "") {
    # read the data from the most recent log_file in check.dir
    log_files <- list.files(path = check.dir, pattern = paste0(files_type,"_files_log_"), recursive = F, full.names = T)
