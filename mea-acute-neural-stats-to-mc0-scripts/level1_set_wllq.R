level1_set_wllq <- function(dat1, standard_analysis_duration_requirement = T) {
  ...
  # initialize default
  dat1[run_type == 'baseline', `:=`(wllq = 1, wllq_notes = '')]
  ...
  dat1[low_ae_flag == 1, `:=` (wllq = 0, wllq_notes = "Baseline # of AE < 10; ")]
  ...
  dat1[run_type == 'baseline', 
       ...
       by = .(experiment.date, plate.id, well)]
  dat1[high_mfr_flag == 1, `:=`(wllq = 0, 
                                wllq_notes = paste0(wllq_notes, "Baseline MFR > ",mfr_upper_threshold," Hz; "))]
  ...
  dat1[run_type == 'baseline', 
       ...
       by = .(experiment.date, plate.id, well)]
  dat1[low_mfr_flag == 1, `:=`(wllq = 0,
                               wllq_notes = paste0(wllq_notes, "Baseline MFR < ",mfr_lower_threshold," Hz; "))]
  ...
  if (standard_analysis_duration_requirement) {
    ...
    if (nrow(dat1[abs(analysis_duration - 2400) > 1400]) > 0){
      cat('The following files have recording length below 1000s or above 3800s. Wllq will be set to 0:\n')
