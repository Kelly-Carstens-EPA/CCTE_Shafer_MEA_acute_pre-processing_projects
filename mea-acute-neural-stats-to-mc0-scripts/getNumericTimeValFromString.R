...
#'
#' @examples
getNumericTimeValFromString <- function(dat.neural.stats, time_string_col, new_cols_suffix) {
  
  org.ncols <- ncol(dat.neural.stats)
  
  # Determine which files include seconds
  dat.neural.stats[, time_includes_sec := grepl('[0-9]+\\:[0-9]{2}\\:[0-9]{2}',get(time_string_col))]
  
  # Determine if am/pm is given
  dat.neural.stats[, meridiem_given := grepl('[AP]M',get(time_string_col))]
  
  # Get a character string of just the time (not the date)
  dat.neural.stats[, time_char := stri_extract(get(time_string_col), regex = '[0-9\\:]+( [AP]M)*$')]
  ...
  dat.neural.stats[, hour := as.integer(stri_extract(time_char, regex = '^[0-9]{1,2}'))]
  stopifnot(nrow(dat.neural.stats[is.na(hour)]) == 0)
  if (nrow(dat.neural.stats[hour %in% 1 & meridiem_given == FALSE]) > 0) {
  ...
  dat.neural.stats[meridiem_given == FALSE & time_includes_sec == TRUE, time_format := '%H:%M:%OS'] # format = Decimal hours (24 hour):Decimal minute:Decimal second
  dat.neural.stats[meridiem_given == FALSE & time_includes_sec == FALSE, time_format := '%H:%M']
  dat.neural.stats[meridiem_given == TRUE & time_includes_sec == TRUE, time_format := '%I:%M:%OS %p'] # format = Decimal hours (12 hour):Decimal minute:Decimal second AM/PM
  dat.neural.stats[meridiem_given == TRUE & time_includes_sec == FALSE, time_format := '%I:%M %p']
  
  # Get sortable time for all (as numeric)
  dat.neural.stats[, time_posix_num := as.numeric(as.POSIXlt(time_char, format = time_format))] 
  stopifnot(nrow(dat.neural.stats[is.na(time_posix_num)]) == 0)
  
  # Add suffix to added column names
  added.cols <- names(dat.neural.stats)[(org.ncols+1):ncol(dat.neural.stats)]
  setnames(dat.neural.stats, old = added.cols, new = paste0(added.cols,'_',new_cols_suffix))
  
  # message & return
  cat('The following columns have been added to dat.neural.stats:\n',
