...
View(dat[is.na(wllt) & !(grepl('LDH',acnm) & treatment %in% ignore.ldh.treatments), .N, by = .(treatment)])
...
# So, perhaps we can leave wllq as-is, knowing that TCPL will ignore wells with NA rval regardless.
...
# Data usability checks - controls MFR within +/- 2 SD of mean
...
acn.dat.description <- paste0('incomplete')
cat(acn.dat.description)
save(dat, acn.dat.description, 
     file = file.path(project_name, "output", paste0(project_name,"_dat.RData")))
```
...
I think that this approach was developed mostly to justify removing several low-activity control wells from the APCRA2019/DNT2019 projects.
...
* I'm not sure that it makes sense to set the wllq to 0 for the LDH and AB parameters based on the MFR.

Alternatively, maybe we could use the data quality threshold from the NFA (exclude plates where plate-wise median of controls on DIV 12 is < 10 spikes per min or < 2 active electrodes).


Replicating what was done for previous projects with TSCA2019 integrated:


```{r}
# Load data sets that have already been pipelined
# previously, 'dat4' was the final output for the run_me for each project
# ** WLLQ was NOT set to 0 based on the below check in these files
...
# 1: APCRA2019  -27.00949 32.33119 6
# 2:   DNT2019  -27.00949 32.33119 8
# 3:  TSCA2019  -27.00949 32.33119 2
# only a few
...
alldat[acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean" & wllt == 'n' & wllq == 1 & rval > wlltn_upper_bound,
       .N, by = .(project, wlltn_mean, wlltn_SD)]
#      project wlltn_mean wlltn_SD  N
# 1: APCRA2019  -27.00949 32.33119  4
# 2:  TSCA2019  -27.00949 32.33119 19
...
So, it looks like the cutoffs with TSCA2019 (orange) are less stringent than those without TSCA2019 (black). So if we used this approach, then the wllq for some old data should be updated to wllq := 1


```{r}
# Compare these wllq settings with previous data ---------------

# Update the wllq in alldat table
alldat[, control_mfr_too_low := any(acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean" & wllt == 'n' & rval < wlltn_lower_bound), 
       by = .(experiment.date, plate.id, rowi, coli)]
alldat[, control_mfr_too_high := any(acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean" & wllt == 'n' & rval > wlltn_upper_bound), 
       by = .(experiment.date, plate.id, rowi, coli)]
alldat[control_mfr_too_low == TRUE, `:=`(wllq = 0,
                                         wllq_notes = paste0('DMSO % change MFR > 2*SD from the mean; ',wllq_notes))]
alldat[control_mfr_too_high == TRUE, `:=`(wllq = 0,
                                          wllq_notes = paste0('DMSO % change MFR > 2*SD from the mean; ',wllq_notes))]


# Load previous mc0
load('lvl0_snapshots/mea_acute_lvl0_2020-07-29.RData')
# [1] "spid" "apid" "rowi" "coli" "conc" "acnm" "wllt" "wllq" "srcf" "rval"
# wllq ntoes, etc not included
nrow(mea_acute_lvl0) == nrow(alldat[project != 'TSCA2019'])# TRUE
mea_acute_lvl0[, .N, by = .(wllq)]
#    wllq      N
# 1:    1 526522
# 2:    0 113468
alldat[project != 'TSCA2019', .N, by = .(wllq)]
#    wllq      N
# 1:    1 527242
# 2:    0 112748
113468 - 112748 # 720
...
So 720 data rows (corresponding to all parameters) have wllq == 0 in the current mc0 that would be updated to wllq == 1 if we used this approach.

```{r}

rm(alldat, all.dat.prev, comp.tb)


# If decide to update wllq based on these threshold for the TSCA2019 dat specifically
all_controls_mfr_lower_bound <- alldat[acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean", unique(wlltn_lower_bound)]
all_controls_mfr_upper_bound <- alldat[acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean", unique(wlltn_upper_bound)]
dat[acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean", wlltn_lower_bound := all_controls_mfr_lower_bound]
dat[acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean", wlltn_upper_bound := all_controls_mfr_upper_bound]
dat[, control_mfr_too_low := any(acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean" & wllt == 'n' & rval < wlltn_lower_bound, na.rm = T), 
    by = .(experiment.date, plate.id, rowi, coli)]
dat[, control_mfr_too_high := any(acnm == "CCTE_Shafer_MEA_acute_firing_rate_mean" & wllt == 'n' & rval > wlltn_upper_bound, na.rm = T), 
    by = .(experiment.date, plate.id, rowi, coli)]
dat[control_mfr_too_low == TRUE, `:=`(wllq = 0,
                                      wllq_notes = paste0('DMSO % change MFR > 2*SD from the mean; ',wllq_notes))]
dat[control_mfr_too_high == TRUE, `:=`(wllq = 0,
                                       wllq_notes = paste0('DMSO % change MFR > 2*SD from the mean; ',wllq_notes))]
dat[wllq_notes %in% 'DMSO % change MFR > 2*SD from the mean; ', .(length(unique(full_well_id)))]
...
dat[wllq_notes == 'DMSO % change MFR > 2*SD from the mean; ', .(length(unique(full_well_id))), by = .(rval < wlltn_lower_bound)]
#     rval V1
# 1:    NA 25
# 2: FALSE 19
# 3:  TRUE  2
...
plotdat[grepl('fir',acnm) & culture.date == 20210811, .N, by = .(wllq, wllq_notes)]
plotdat[is.na(wllq)]
```
...
# [1] "200C12" "201A07" "201C05" "201C06" "201C07" "201D03" "201D06" "201E06" "201F01" "201F03" "201F05" "201G01" "201G02" "201H04" "0" 
```

These treatments will not have any dose-response curves generated in TCPL. Confirm with lab technicians that these compounds were not retested elsewhere and that it is okay that we don't have any data for these.
...
# yes, this checks out (doesn't look like I missed any data)
```


**Range of culture dates:***
```{r}
dat[, range(culture.date)]
```
...
# ** check for additional columns in dat to save
