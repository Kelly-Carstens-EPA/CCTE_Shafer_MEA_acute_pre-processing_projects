...
```{r}
# Note that paths can be written relative to the .Rproj
...
project.output.dir <- file.path(project_name) # where the "output" folder should be created
...
## Setting wllq based on recording information
...
Since this spidmap does not contain the stock concentrations, get a table including spid's and actual concentrations tested from invitrodb

`eval = FALSE` because only need to run once
```{r eval = FALSE}
library(RMySQL)
con <- dbConnect(drv = MySQL(), user = Sys.getenv('INVITRODB_USER_RO'), 
                 pass = Sys.getenv('INVITRODB_PASS_RO'),
                 host = Sys.getenv('INVITRODB_HOST'),
                 db = 'invitrodb') # user, pass, etc saved under environmental variables (.Renviron). Modify as needed
all.samples.tb <- dbGetQuery(con, 'SELECT * FROM sample')
setDT(all.samples.tb)
dbDisconnect(con)
TSCA2019.samples.tb <- all.samples.tb[spid %in% spidmap$spid]
write.csv(TSCA2019.samples.tb , file = file.path(project.output.dir,'TSCA2019_sample_tb_from_invitrodb_2023-08-02.csv'))
```

Merge information from `TSCA2019_sample_tb_from_invitrodb_2023-08-02.csv` with the spidmap

```{r}
TSCA2019.samples.tb <- read.csv(file.path(project.output.dir,'TSCA2019_sample_tb_from_invitrodb_2023-08-02.csv'))
spidmap <- merge(spidmap, TSCA2019.samples.tb, by = 'spid', all.x = T) # stkc = stock concentration
...
dat[wllt == 't', .N, by = .(concs_decent_agreement = conc_log10_diff_abs < 0.005)]
...
Updated my_anticipated_conc for G16 and recheck:
...
# With previous script configuration, 'dat4' was the final output for the run_me for each project,
# except that WLLQ had NOT yet be set to 0 for outlier DMSO wells. 
# WLLQ was set to 0 for outlier DMSO wells in mea-acute-neural-stats-to-mc0-scripts/deprecated_create_lvl0_snapshots.R.
# The output from that script saved 2 tables under lvl0_snapshots/:
# lvl0_snapshots/dat4_2020-07-29.RData and 
# lvl0_snapshots/mea_acute_lvl0_2020-07-29.RData
...
save(dat, acn.dat.description, 
     file = file.path(project.output.dir, "output", paste0(project_name,"_MEA_Acute_for_tcpl_lvl0.RData")))
```
