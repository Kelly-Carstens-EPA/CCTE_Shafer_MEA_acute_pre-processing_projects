...
if (select.raw.cytotox.files) {
  ...
  # Check that there are exactly 3 CTB files per group
  num.CTB.per.group <- sapply(raw.cytotox.files, function(seti) sum(!grepl('LDH',unlist(seti))))
  cat('Groups that do not have exactly 3 CTB file:')
  num.CTB.per.group[num.CTB.per.group != 3]
...
Note assumptions for LDH and CTB raw data file reading functions:
...
The function `read_acute_CTB_raw_data` assumes that:
* 1 plate per file
...
Read raw data from CTB files
```{r}
raw.cytotox.files.CTB <- raw.cytotox.files[!grepl('LDH',raw.cytotox.files)]
rawdat.CTB <- data.table()
for (filei in raw.cytotox.files.CTB) {
  add.tb <- read_acute_CTB_raw_data(filei)
  rawdat.CTB <- rbind(rawdat.CTB, add.tb)
}

# # To debug an individual file:
# filei
# debugonce(read_acute_CTB_raw_data)
# add.tb <- read_acute_CTB_raw_data(filei)

# Note any CTB files that could not be read (manually check that these data values have been correctly copied over to the Calculations file)
setdiff(basename(raw.cytotox.files.CTB), rawdat.CTB$filename)
```

Confirm expected # of raw data values per file

```{r}
rawdat.CTB[, .N, by =.(filename)][N != 96]
rawdat.LDH[, .N, by =.(filename)][N != 96*3]
```


Get plate IDs for raw CTB and LDH data, then combine
...
# (1-2 digits)-(3-4 digits)
rawdat.CTB[, plate.id := stri_extract(filename, regex = '[0-9]{1,2}\\-[0-9]{3,4}')]
rawdat.CTB[, plate.id := paste0('MW',plate.id)]
rawdat.CTB[, .N, by = .(plate.id)] # confirm these looks correct


# Combine cyto data
rawdat.cytotox <- rbind(rawdat.LDH, rawdat.CTB, fill = T)
```
...
# (if any cases, could be that there was a typo in the plate.id in the filename for CTB or in the file header for LDH. Check it out and correct as needed)
```
...
* For G11, reverse the plate orientation for CTB Plate 71-7015 (not implemented in raw or calc file) *** (flip 180 degrees. Well A8 -> F1)
* For G20 plate 75-8201, reverse the plate orientation for CTB (not implemented in raw or calc file) *** (flip 180 degrees. Well A8 -> F1)
...
rawdat.cytotox[group_int == '11' & plate.id == 'MW71-7015' & acsn == 'CTB' 
               & rowi %in% c(1:6) & coli %in% c(1:8), 
               `:=`(rowi = 7 - rowi_org,
                    coli = 9 - coli_org)]
rawdat.cytotox[group_int == '20' & plate.id == 'MW75-8201' & acsn == 'CTB'
               & rowi %in% c(1:6) & coli %in% c(1:8), 
               `:=`(rowi = 7 - rowi_org,
                    coli = 9 - coli_org)]
```

Visually confirm G11 MW71-7015 looks correct

```{r}
test.plate <- dcast(rawdat.cytotox[group_int == '11' & plate.id == 'MW71-7015' & acsn == 'CTB'], rowi ~ coli, value.var = 'well_org')
test.plate
```

## Calculate blank-corrected values

Calculate average of blank well.s Blank wells in both assays are in G1, 2 and 3
...
rawdat.cytotox[acsn == 'CTB' & rowi %in% c(1:6) & coli %in% c(1:8), use_wells:=1]
rawdat.cytotox[acsn == 'LDH' & ((rowi %in% c(1:6) & coli %in% c(1:8)) |
                                  (rowi == 7 & coli %in% c(4,5)) |
                                  (rowi == 8 & coli %in% c(1:6))), use_wells := 1]
rawdat.cytotox <- rawdat.cytotox[use_wells == 1]

# Prepare to merge
cat(intersect(names(cytodat), names(rawdat.cytotox)), sep = '", "')
cytodat[, in_calc := 1]
rawdat.cytotox[, in_raw := 1]
nrow(cytodat)
nrow(rawdat.cytotox)
comp.dat <- merge(cytodat, rawdat.cytotox, by = c("plate.id", "rowi", "coli", "rval", "acnm",  "culture_folder", "group_char", "group_int", "culture.date"),
...
For G12, in the lab notebook, plate 1 was originally written as 75-7519. Then it was crossed out and replaced with 75-5819 on 3/23/21 and initialed by Theresa. Therefore, I think 75-7519 in the Calculations file was a typo. Will replace that now

```{r}
cytodat[culture.date == '20210310' & group_int == 12 & plate.id == 'MW75-7519', plate.id := 'MW75-5819']
```

For G36 - lab notebook and neural stats compiler files indicate that the plate.id should be 75-9201, not 75-9121, so we'll go with that.

```{r}
rawdat.cytotox[group_int == 36 & plate.id == 'MW75-9121', plate.id := 'MW75-9201']
```

Re-merge

```{r}
comp.dat <- merge(cytodat, rawdat.cytotox, by = c("plate.id", "rowi", "coli", "acnm",  "culture_folder", "group_char", "group_int", "culture.date"),
                  suffixies = c('.calc','.raw'), all = T)
...
```
Ah, yes, this is the plate where there were only two 1/2 lysis control wells instead of the usual 3.
So this is as expected

Compare rvals

```{r}
comp.dat[, rval.rnd.x := signif(rval.x, 3)]
comp.dat[, rval.rnd.y := signif(rval.y, 3)]
comp.dat[, rval_pct_diff := abs(rval.x - rval.y)/((rval.x + rval.y)*0.5)*100]

comp.dat[rval.rnd.x != rval.rnd.y, .N, by = .(acnm)]

comp.dat[rval.rnd.x != rval.rnd.y, .N, by = .(culture.date, plate.id)]
```


Where we expect to see differences

* For G11, reverse the plate orientation for CTB Plate 71-7015 (not implemented in raw or calc file) *** (flip 180 degrees. Well A8 -> F1)
* For G20 plate 75-8201, reverse the plate orientation for CTB (not implemented in raw or calc file) *** (flip 180 degrees. Well A8 -> F1)
* G20 and G21 - raw files are right, just weren't copy & pasted into calc files

```{r}
comp.dat[, differences_expected := 0]

# differences in plate orientation, fixed in rawdat
comp.dat[group_int == 11 & grepl('AB',acnm) & plate.id == 'MW71-7015', differences_expected := 1]
comp.dat[group_int == 20 & grepl('AB',acnm) & plate.id == 'MW75-8201', differences_expected := 1]

# known lack of copying files into calc files
comp.dat[group_int %in% c(20,21), differences_expected := 1]

# rvals may differ by a small amont due to rounding - this if fine
# wnat to view cases with more significant differences
comp.dat[, .N, by = .(rvals_agree = rval_pct_diff < 0.5, 
                      differences_expected)]
#    rvals_agree_v2 differences_expected     N
# 1:           TRUE                    0 10665
# 2:          FALSE                    1   435
# 3:           TRUE                    1   237
# 4:          FALSE                    0   204
...
So there are 435 cases were differences are not expected, but are present. Let's confirm that the rawdat.cytotox is correct here (rather than some other issue I wasn't aware of)

```{r}
comp.dat[differences_expected == 0 & rval_pct_diff >= 0.5, .(.N, affected = paste0(unique(acsn),collapse = ","), max_diff = max(rval_pct_diff)), by = .(group_int, culture.date, plate.id)][order(culture.date, group_int)]
#    group_int culture.date  plate.id  N affected    max_diff
# 1:         1     20201104 MW71-7106 17      LDH 2955.811277
# 2:         6     20210120 MW72-8208 18      LDH 1199.556541
# 3:         7     20210120 MW72-8212 20      LDH 3424.811219
# 4:        13     20210310 MW75-8003  1      LDH  200.000000
# 5:        19     20210414 MW75-8114 53      LDH  200.000000
# 6:        26     20210526 MW75-8219 47       AB    4.042701
# 7:        31     20210630 MW78-4406 48       AB  200.000000
...
* Compare raw & calc file discrepancies. Probably will default to raw, but wanna check
* Hmm, that seems like a lot of zero's for the LDH... is this right? Is this an indication of badness fro the assay? We'll check it out
...
* Look at distribution of lysis control wells (need to develop a cutoff, as Kathleen said 5/31)
