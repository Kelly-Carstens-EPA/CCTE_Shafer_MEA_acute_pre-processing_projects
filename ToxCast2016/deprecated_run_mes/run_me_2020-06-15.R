...
# 06/10/2020
# update the AB values to set negative values to zero
load(file = paste0("output/ToxCast2016_cytodat_from_conc_resp_log_2020-06-08.RData"))
cytodat[grepl("AB",acsn) & rval < 0, rval := 0.0]
save(cytodat, file = paste0("output/",dataset_title,"_cytodat_from_conc_resp_log_",as.character.Date(Sys.Date()),".RData")) # save the updated cytodat

# fix plate.id in 20151208, digit flip typo in neural stats files
load(file = paste0("output/",dataset_title,"_dat2_2020-06-06.RData"))
dat2[experiment.date == "20151208", unique(plate.id)] # "MW1068-36" "MW1068-37" "MW1068-38"
dat2[experiment.date == "20151208", plate.id := sub("MW1068","MW1086",plate.id)]
dat2[experiment.date == "20151208", unique(plate.id)] # "MW1086-36" "MW1086-37" "MW1086-38"

# In 20151201, plate 1086-26 is named 1086-25 (I know this is wrong, because there is a 1086-25 in exp date 20151203)
dat2[experiment.date == "20151201" & plate.id == "MW1086-25", plate.id := "MW1086-26"]
save(dat2, file = paste0("output/",dataset_title,"_dat2_",as.character.Date(Sys.Date()),".RData"))
...
dat4[(acsn %in% c("NHEERL_MEA_acute_LDH","NHEERL_MEA_acute_AB")), unique(treatment)]
dat4[treatment == "LYSIS" & acsn %in% c("NHEERL_MEA_acute_LDH"), wllt := "p"]
dat4[treatment == "LYSIS" & acsn %in% c("NHEERL_MEA_acute_AB"), wllt := "m"] # loss of signal control for AB viability assay
...
(wllq_summary <- dat4[wllq == 0 & !wllq_notes == "rval is NA; ", .(treatment = paste0(unique(treatment),collapse=","), wllq_notes = paste0(unique(wllq_notes), collapse = ""), endpoints = paste0(unique(endpoint_type),collapse=","), srcf = paste0(unique(srcf),collapse=",")), 
...
# save dat4
dat4 <- dat4[, .(treatment, spid, experiment.date, plate.id, apid, rowi, coli, conc, acsn, wllt, wllq, wllq_notes, rval, srcf, dat2, dat3)]
