...
for (aeidi in aeids) {
  ...
  graphics.off()
  pdf(file = fname, height = 6, width = 10)
  par(mfrow=c(1,1))
...
ls()
mc5_mc6[spid %in% use.spid, .(spid, aenm, hitc)]
mc5_mc6[spid %in% use.spid, .(num_hits = sum(hitc==1), num_no_hits = sum(hitc==0)), by = .(spid, chnm)]
# huh... some hits, som enot
mc5_mc6[chnm %in% c('Chlordane','Dieldrin','Endosulfan','Endrin',
                    'Heptachlor','Heptachlor epoxide B','Lindane'), .(num_hits = sum(hitc==1), num_no_hits = sum(hitc==0)), by = .(spid, chnm)][order(chnm)]
mc5_mc6[chnm == 'Chlordane' & hitc == 1, .(chnm, aenm, hitc, modl, modl_ga, mc6_mthd)]

# which endpoints are seeing the most hits for the organochlorines?
mc5_mc6[chnm %in% c('Chlordane','Dieldrin','Endosulfan','Endrin',
                    'Heptachlor','Heptachlor epoxide B','Lindane') & hitc == 1, .N, by = .(aenm)][order(-N)]
sbox_mc5_mc6 <- mc5_mc6

sbox_mc5_mc6[, hitc.f := ifelse(!(fitc %in% c(36,45)) & (is.na(flag_length) | flag_length < 3),
                                hitc,
                                0)]

# comparing to current normalization method
load('C:/Users/ACARPE01/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/mea_acute/data/invitrodb_2020-12-04.RData')
mc5_mc6[, hitc.f := ifelse(!(fitc %in% c(36,45)) & (is.na(flag_length) | flag_length < 3),
                           hitc,
                           0)]
usecols <- c('spid','chnm','aenm','hitc','hitc.f','fitc','mc6_mthd','modl','modl_ga')
comp.hitc <- merge(mc5_mc6[, .SD, .SDcols = c('aeid',usecols)], 
      sbox_mc5_mc6[, .SD, .SDcols = usecols],
      by = c('spid','chnm','aenm'), suffixes = c('.c','.n'))

comp.hitc[chnm %in% c('Chlordane','Dieldrin','Endosulfan','Endrin',
                      'Heptachlor','Heptachlor epoxide B','Lindane'), .(num_hit_cur = sum(hitc.c == 1),
                                                                        num_hit_new = sum(hitc.n == 1),
                                                                        num_hit_filtered_cur = sum(hitc.f.c == 1),
                                                                        num_hit_filtered_new = sum(hitc.f.n == 1)),
          by = .(spid, chnm)][order(chnm)]
# spid                 chnm num_hit_cur num_hit_new num_hit_filtered_cur num_hit_filtered_new
# 1:  TP0001411E05            Chlordane           3           3                    3                    3
# 2:  TP0001412F07             Dieldrin          10          10                   10                   10
# 3: EPAPLT0167G10           Endosulfan           9           9                    5                    6
# 4:  TP0001413H09           Endosulfan           3           3                    3                    3
# 5:  TP0001414E02               Endrin           8           7                    6                    7
# 6:  TP0001411C06           Heptachlor          10          10                   10                   10
# 7:  TP0001412F11 Heptachlor epoxide B           9           9                    9                    9
# 8:  TP0001413B01              Lindane          10          10                   10                   10
# only 1 change?? wow. (endrin)
# filtered hits are looking a lot different.. actually pretty good

comp.hitc[chnm %in% c('Chlordane','Dieldrin','Endosulfan','Endrin',
                      'Heptachlor','Heptachlor epoxide B','Lindane') & hitc.f.c != hitc.f.n]

comp.hitc[, .N, by = .(hitc.c, hitc.n)]
# hitc.c hitc.n    N
# 1:     -1     -1  129
# 2:      0      0 6593
# 3:      1      1  631
...
comp.hitc[, .N, by = .(hitc.f.c, hitc.f.n)]
# hitc.f.c hitc.f.n    N
# 1:       -1       -1  129
# 2:        0        0 7181
# 3:        1        0   30
...
# visualizing  chlordane and endosulfant activity rval
comp.hitc[spid == 'TP0001411E05' & hitc.f.n == 1]
comp.hitc[spid == 'TP0001413H09' & hitc.f.n == 1]

# up hit counts in general
comp.hitc[, .(num_hit_cur = sum(hitc.c == 1),
              num_hit_new = sum(hitc.n == 1),
              num_hit_filtered_cur = sum(hitc.f.c == 1),
              num_hit_filtered_new = sum(hitc.f.n == 1)), by = .(aenm)][order(-num_hit_filtered_new)]

# checking out the other potential up positives
comp.hitc[spid %in% use.spid & !(chnm %in% c('Chlordane','Dieldrin','Endosulfan','Endrin',
                      'Heptachlor','Heptachlor epoxide B','Lindane')), .(num_hit_cur = sum(hitc.c == 1),
                                                                        num_hit_new = sum(hitc.n == 1),
                                                                        num_hit_filtered_cur = sum(hitc.f.c == 1),
                                                                        num_hit_filtered_new = sum(hitc.f.n == 1)),
          by = .(spid, chnm)][order(chnm)]
# spid         chnm num_hit_cur num_hit_new num_hit_filtered_cur num_hit_filtered_new
# 1: TP0001413C06       Kepone           1           1                    1                    1
# 2: TP0001413G10 Methoxychlor           0           0                    0                    0
# 3: TP0001413B03        Mirex           0           0                    0                    0
# 4: TP0001411C10     Nicotine           1           1                    1                    1
# 5: TP0001414F11   Permethrin           0           0                    0                    0

comp.hitc[chnm == 'Kepone'] # only hits is CCTE_Shafer_MEA_acute_burst_percentage_std_up
comp.hitc[chnm %in% c('Methoxychlor','Permethrin')]
# i'm seeing several flags for noise... only 1 flag 12 for border miss
# looks like just didn't get above baseline

comp.hitc[chnm == 'Nicotine']

# how do the Oc's rank as far as most up hits compared to all chem?
comp.hitc[, .(num_hit_cur = sum(hitc.c == 1),
                                                                                                num_hit_new = sum(hitc.n == 1),
                                                                                                num_hit_filtered_cur = sum(hitc.f.c == 1),
                                                                                                num_hit_filtered_new = sum(hitc.f.n == 1)),
          by = .(spid, chnm)][order(-num_hit_filtered_new)][1:20]
comp.hitc[chnm == 'Chlorpyrifos oxon' & hitc.f.n == 1]
