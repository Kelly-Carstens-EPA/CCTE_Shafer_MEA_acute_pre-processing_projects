...
cmc5[hitc.org == 1 & hitc.new == 0 & any_bi_flags.new == FALSE & any_ba_flags.org == FALSE]
# 3 cases
...
plot(bval.new ~ bval.org, compare.bval)
abline(0,1)
...
aeidi <- 2425
apidi <- "20190411"
apid.check <- mc3[apid == apidi]
apid.check[wllt == "n", cndx := 0]
boxplot(cval ~ cndx, apid.check[aeid == aeidi], main = paste0(mc5_mc6[aeid == aeidi, unique(aenm)], " Changes in bval for 1 apid"))
...
cmc5[spid %in% spids.tested & aeid == aeidi & spid != "DMSO", .(spid, chnm, hitc.org, hitc.new, aeid)]
