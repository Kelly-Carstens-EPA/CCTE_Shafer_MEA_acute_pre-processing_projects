...
# same as 12-09, but this time using 99th percentile as upper bound
...
mc0[,
    ...
    by = list(acid, spid)]
mc0[,
    pval := quantile(max_med_rval[wllt == 't'], probs = 0.99, na.rm = TRUE),
    by = list(acid)]
...
for (i in up_aeids){
  tcplMakeAeidPlts(i,
                   lvl=6L, 
                   fname = paste0('AEID',paste(i,'L6',aeid.info[aeid==i,aenm],as.character.Date(Sys.Date()),'b',sep='_'),'.pdf'),
                   odir=getwd())
}
...
mc6_collapsed <- mc6[, .(mc6_mthd = paste0(unique(mc6_mthd_id), collapse = ","), flag_length = length(unique(flag)), mc6_mthd = paste0(unique(mc6_mthd_id), collapse = ",")), by = c("spid","aeid")]
...
# save the data
sbox_run_summary <- paste0("Pre-normalizing mea acute up endpoints with pval as 99th percentiles of max_med_rval where wllt=='t', rval := (rval - bval)/(pval - bval)*100.\nLvl0: mea_acute_lvl0_2020-07-29.Rdata.\nDate ran: 2020-12-10.")
save(sbox_run_summary, mc3_mthds, mc4_mthds, mc6_mthds, mc3, mc5_mc6, 
     file = file.path(paste0("C:/Users/ACARPE01/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/mea_acute/normalization_ideas_Dec2020/sbox_run/sbox_run_",as.character.Date(Sys.Date()),"b",".RData")))
dbDisconnect(con)
